...
ThemeManager::QtFallbackInputBarEffect resolvedQtFallbackInputBarEffect()
{
    const ThemeManager::QtFallbackInputBarEffect effect =
            ThemeManager::instance().qtFallbackInputBarEffect();
    if (effect == ThemeManager::QtFallbackInputBarEffect::LiquidGlass
        && !QtFallbackLiquidGlassController::liquidGlassSupported()) {
        return ThemeManager::QtFallbackInputBarEffect::GaussianBlur;
    }
    return effect;
}
        ...
        : QWidget(parent)
{
    ...
    updateQtFallbackEffectMode();
    m_liquidGlass->setShape(QRectF(rect()), CORNER_RADIUS);
...
void FloatingInputBar::updateQtFallbackEffectMode()
{
    if (!m_liquidGlass) {
        return;
    }

    switch (resolvedQtFallbackInputBarEffect()) {
    case ThemeManager::QtFallbackInputBarEffect::LiquidGlass:
        m_liquidGlass->setEffectMode(QtFallbackLiquidGlassController::EffectMode::LiquidGlass);
        break;
...
bool FloatingInputBar::shouldUseQtFallbackLiquidGlass() const
{
    return !m_usesNativeGlass
            && resolvedQtFallbackInputBarEffect() != ThemeManager::QtFallbackInputBarEffect::Default;
}
...
void FloatingInputBar::paintQtFallbackLiquidGlass(QPainter& painter, const QRectF& panelRect)
{
    ...
    m_liquidGlass->setShape(panelRect, CORNER_RADIUS);
